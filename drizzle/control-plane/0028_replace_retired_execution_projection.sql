-- Replace the retired JSON execution projection with the canonical living graph.
-- These tables are derived entirely from accepted TreeDX/governance sources and
-- active workday/communication records, so no authoritative work is discarded.

DROP TABLE IF EXISTS execution_edges;
DROP TABLE IF EXISTS execution_graph_revisions;
