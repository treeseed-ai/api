...
export function junit(report) {
    ...
    return [
        ...
        ...report.results.map((result) => result.ok
            ? `  <testcase classname="api.acceptance" name="${escape(result.id)}" time="${result.durationMs / 1000}" />`
            : `  <testcase classname="api.acceptance" name="${escape(result.id)}" time="${result.durationMs / 1000}"><failure>${escape(result.failures.join('\\n'))}</failure></testcase>`),
        `</testsuite>`,
    ].join('\n');
}
