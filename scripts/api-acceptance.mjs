...
function fixtureValue(name) {
	const map = {
		...
		inputId: '${fixtures.decisionExecutionInput.id}',
		capacityPlanId: '${fixtures.capacityPlan.id}',
		workdayId: '${fixtures.workday.id}',
...
function bodyForFactory(factory, descriptor, actor) {
	...
	const byFactory = {
		...
		providerCheckIn: {
			...
			status: 'open',
			executionProviders: [],
			capabilities: ['acceptance-dry-run'],
			nativeLimits: { source: 'acceptance' },
		...
		agentModeRun: {
			status: 'succeeded',
			selectedInput: { acceptance: true },
			outputs: { ok: true },
			usageActual: { actualCredits: 0 },
		},
		providerNextAssignment: {
			runnerId: '${fixtures.provider.id}:acceptance-runner',
			leaseSeconds: 30,
			capabilities: ['acceptance-dry-run'],
		},
		providerAssignmentRenew: {
			runnerId: '${fixtures.provider.id}:acceptance-runner',
			leaseToken: 'acceptance-invalid-token',
			leaseSeconds: 30,
		},
		providerAssignmentReturn: {
			runnerId: '${fixtures.provider.id}:acceptance-runner',
			leaseToken: 'acceptance-invalid-token',
			reason: 'acceptance route descriptor proof',
		},
		providerAssignmentComplete: {
			runnerId: '${fixtures.provider.id}:acceptance-runner',
			leaseToken: 'acceptance-invalid-token',
			output: { acceptance: true },
		},
		providerAssignmentFail: {
			runnerId: '${fixtures.provider.id}:acceptance-runner',
			leaseToken: 'acceptance-invalid-token',
			code: 'acceptance_failure',
			message: 'Acceptance failure fixture.',
			retryable: false,
		},
		...
		capacityAllocationSet: {
			version: `acceptance-${stamp}`,
			policy: { source: 'acceptance' },
			slices: [{ projectId: '${fixtures.project.id}', capacityProviderId: '${fixtures.provider.id}', percent: 100 }],
		},
		providerAssignment: {
			projectId: '${fixtures.project.id}',
			capacityProviderId: '${fixtures.provider.id}',
			projectAgentClassId: '${fixtures.projectAgentClass.id}',
			mode: 'planning',
			capacityEnvelope: { mode: 'planning', teamId: '${fixtures.team.id}', projectId: '${fixtures.project.id}' },
			decisionInput: { acceptance: true },
		},
		...
		projectAgentClass: { slug: `acceptance-${actor}`, name: `Acceptance ${actor} Agent`, allowedModes: ['planning', 'acting'] },
		planningInputRequest: {
			projectId: '${fixtures.project.id}',
			projectAgentClassId: 'acceptance-agent-class',
			mode: 'planning',
			prompt: 'Acceptance planning input request.',
		},
		...
		agentCapacityPlan: {
			projectId: '${fixtures.project.id}',
			allocationSetId: '${fixtures.allocationSet.id}',
			metadata: { acceptance: true },
		},
		decisionExecutionRevision: { reason: 'Acceptance revision request.' },
		workdayCapacityEnvelope: {
			id: '${fixtures.workday.id}',
			projectId: '${fixtures.project.id}',
			environment: '${environment}',
			availableCredits: 1,
		...
		githubActionsSecretDeploy: {
			installationId: 'acceptance-installation',
			repository: 'treeseed-acceptance/fixture',
			scope: 'environment',
			environment: 'acceptance',
			secretName: 'TREESEED_ACCEPTANCE_SECRET',
			encryptedValue: 'acceptance-github-encrypted-value',
			keyId: 'acceptance-key',
		},
		workflowOperationDispatch: {},
...
function sdkArgsForMethod(method) {
	...
	const args = {
		...
		importProjectRepository: ['${fixtures.team.id}', {
			ok: true,
			provider: 'github',
			team: '${fixtures.team.slug}',
			repository: {
				owner: 'treeseed-acceptance',
				name: 'import-${runNonce}',
				slug: 'treeseed-acceptance/import-${runNonce}',
				defaultBranch: 'main',
				visibility: 'public',
				htmlUrl: 'https://github.com/treeseed-acceptance/import-${runNonce}',
				cloneUrl: 'https://github.com/treeseed-acceptance/import-${runNonce}.git',
			},
			credentialRef: 'env:TREESEED_GITHUB_TOKEN',
			credential: {
				ref: 'env:TREESEED_GITHUB_TOKEN',
				configured: false,
				source: 'missing',
				repositoryScopedRef: 'env:TREESEED_GITHUB_TOKEN_TREESEED_ACCEPTANCE_IMPORT_${runNonce}',
			},
			architecture: {
				...
				contentPublishTarget: {
					kind: 'cloudflare_r2',
					manifestPath: 'teams/${fixtures.team.slug}/projects/import-${runNonce}/published/common.json',
				},
			...
			plannedRecords: {
				...
				hubRepository: {
					...
					owner: 'treeseed-acceptance',
					name: 'import-${runNonce}',
					url: 'https://github.com/treeseed-acceptance/import-${runNonce}',
					defaultBranch: 'main',
					credentialRef: 'env:TREESEED_GITHUB_TOKEN',
				},
				contentSource: {
				...
				treeDxLibrary: {
					libraryId: '${fixtures.team.slug}/import-${runNonce}',
					contentPath: 'docs',
				},
			},
		}],
		...
		projectSecretEscrow: ['${fixtures.project.id}', 'acceptance-missing-escrow'],
		updateProjectSecretEscrow: ['${fixtures.project.id}', 'acceptance-missing-escrow', {}],
		migrateProjectSecretEscrow: ['${fixtures.project.id}', 'acceptance-missing-escrow', {}],
		tombstoneProjectSecretEscrow: ['${fixtures.project.id}', 'acceptance-missing-escrow'],
		...
		dispatchProjectWorkflowOperation: ['${fixtures.project.id}', 'acceptance-missing-workflow-operation', {}],
		initializeProjectRepository: ['${fixtures.project.id}', 'primary', {}],
		...
		createCapacityAllocationSet: ['${fixtures.team.id}', {
			version: `acceptance-sdk-${stamp}`,
			policy: { source: 'sdk_acceptance' },
			slices: [{ projectId: '${fixtures.project.id}', capacityProviderId: '${fixtures.provider.id}', percent: 100 }],
		}],
		capacityAllocationSet: ['${fixtures.team.id}', '${fixtures.allocationSet.id}'],
		activateCapacityAllocationSet: ['${fixtures.team.id}', '${fixtures.allocationSet.id}'],
		providerAvailabilitySessions: ['${fixtures.team.id}'],
		providerAssignments: ['${fixtures.team.id}'],
		...
		updateProjectAgentClass: ['${fixtures.project.id}', '${fixtures.projectAgentClass.id}', { name: 'Acceptance SDK Agent' }],
		...
		decisionPlanningStatus: ['acceptance-${runNonce}-decision'],
		createPlanningInputRequest: ['acceptance-${runNonce}-decision', {
			projectId: '${fixtures.project.id}',
			projectAgentClassId: 'acceptance-sdk-agent-class',
			mode: 'planning',
			prompt: 'Acceptance SDK planning input request.',
		}],
		decisionExecutionInputs: ['acceptance-${runNonce}-missing-decision'],
		createDecisionExecutionInput: ['acceptance-${runNonce}-decision', {
			projectId: '${fixtures.project.id}',
			projectAgentClassId: 'acceptance-sdk-agent-class',
			mode: 'acting',
			status: 'proposed',
		...
		acceptDecisionExecutionInput: ['acceptance-${runNonce}-decision-input', {}],
		requestDecisionExecutionInputRevision: ['acceptance-${runNonce}-decision-input', { reason: 'Acceptance SDK revision request.' }],
		decisionCapacityPlans: ['acceptance-${runNonce}-decision'],
		createDecisionCapacityPlan: ['acceptance-${runNonce}-decision', {
			projectId: '${fixtures.project.id}',
			allocationSetId: '${fixtures.allocationSet.id}',
			metadata: { acceptance: true },
		}],
		capacityPlan: ['acceptance-${runNonce}-missing-capacity-plan'],
		acceptCapacityPlan: ['acceptance-${runNonce}-missing-capacity-plan', {}],
		requestCapacityPlanRevision: ['acceptance-${runNonce}-missing-capacity-plan', { reason: 'Acceptance SDK revision request.' }],
		scheduleCapacityPlan: ['acceptance-${runNonce}-missing-capacity-plan', { workDayId: '${fixtures.workday.id}' }],
		createWorkday: [{
			id: 'acceptance-${runNonce}-created-workday',
			projectId: '${fixtures.project.id}',
			environment: '${environment}',
			availableCredits: 1,
			modeSplits: { planning: 0.4, acting: 0.6 },
		}],
		workday: ['acceptance-${runNonce}-missing-workday'],
		startWorkday: ['acceptance-${runNonce}-missing-workday'],
		pauseWorkday: ['acceptance-${runNonce}-missing-workday'],
		completeWorkday: ['acceptance-${runNonce}-missing-workday'],
		workdaySummary: ['acceptance-${runNonce}-missing-workday'],
