...
const track = release.includes('-rc.') ? 'development' : 'stable', debianRelease = release.replace(/-rc\.(\d+)$/u, '~rc$1');
const runtime = {
...
const bundle = componentReleaseSchema.parse({
	schemaVersion: 'treeseed.component-release/v1', componentId: 'api', release: debianRelease, track,
