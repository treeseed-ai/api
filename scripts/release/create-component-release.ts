...
if (!release || !sourceCommit || !apiDigest || !runnerDigest) throw new Error('Release, exact source commit, and every multi-architecture image digest are required.');
const digest = /^sha256:[a-f0-9]{64}$/u;
if (!/^[a-f0-9]{40}$/u.test(sourceCommit) || ![apiDigest, runnerDigest].every((value) => digest.test(value))) throw new Error('Source or image digest is malformed.');
...
const definition = parse(readFileSync(resolve('deploy/compose.template.yml'), 'utf8').replaceAll('@API_IMAGE@', `treeseed/api@${apiDigest}`).replace('@RUNNER_IMAGE@', `treeseed/op-runner@${runnerDigest}`));
...
const runtime = {
	...
	services: [
		...
		{ id: 'migration', composeService: 'migration', endpoints: [] },
	...
	configuration: {
		environment: [
			'NODE_ENV', 'TREESEED_API_AUTH_APPROVAL_BASE_URL', 'TREESEED_API_BASE_URL',
			'TREESEED_API_BOOTSTRAP_ADMIN_ALLOWLIST', 'TREESEED_ENVIRONMENT', 'TREESEED_LIBRARY_BRANCH', 'TREESEED_LOCAL_DEV_MODE',
			'TREESEED_LIBRARY_STORAGE_OWNER_TEAM_ID', 'TREESEED_LIBRARY_STORAGE_CONNECTION_ID',
			'TREESEED_MAILPIT_SMTP_HOST', 'TREESEED_MAILPIT_SMTP_PORT', 'TREESEED_SITE_URL', 'TREESEED_TREEDX_JWT_AUDIENCE',
			'TREESEED_TREEDX_JWT_ISSUER', 'TREESEED_TREEDX_URL', 'TREESEED_TREEDX_NODE_ID', 'TREESEED_AI_RUNTIME',
		].map((name) => ({ name, required: false, source: 'configuration' as const })),
		secretEnvironment: [
			'SESSION_SECRET', 'TREESEED_CLOUDFLARE_ACCOUNT_ID', 'TREESEED_CLOUDFLARE_API_TOKEN',
			'TREESEED_CONTENT_BUCKET_NAME', 'TREESEED_GITHUB_TOKEN', 'TREESEED_R2_ACCESS_KEY_ID',
...
const bundle = componentReleaseSchema.parse({
	...
	evidence: { provenance: [tagUrl('treeseed/api'), tagUrl('treeseed/op-runner')], sboms: [tagUrl('treeseed/api'), tagUrl('treeseed/op-runner')], vulnerabilities: [] },
});
...
console.log(JSON.stringify({ ok: true, release, sourceCommit, apiDigest, runnerDigest, runtimeDigest: bundle.runtimeDigest }));
