...
try {
	...
	if (error && typeof error === 'object' && 'code' in error && error.code === 'ERR_MODULE_NOT_FOUND') {
		if (entrypointCheckOnly) {
			process.stderr.write('Treeseed api verify: @treeseed/sdk is required for verify entrypoint resolution.\n');
			process.exit(1);
		}
		if (process.env.TREESEED_VERIFY_DRIVER === 'act') {
			process.stderr.write('Treeseed api verify: `act` mode requires @treeseed/sdk to be installed.\n');
			process.exit(1);
		}
		runDirectVerify();
	}
