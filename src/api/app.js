...
async function loadProjectHostBindingContext({ store, runtime, principal, details }) {
	...
	const [launchRequirements, managedHostsRaw, teamHosts, repositoryHosts] = await Promise.all([
	...
	const managedHosts = project.metadata?.acceptance === true
		? managedHostsRaw.map((host) => host.id === 'treeseed-managed-web'
			? {
				...host,
				status: 'active',
				metadata: {
