...
export class ProjectAgentClassRepository {
	...
	async create(value: ProjectAgentClass, operations: CapacityDatabaseOperation[] = [], now = new Date().toISOString()): Promise<ProjectAgentClass> {
		...
		await this.database.batch([
			{ query: `INSERT INTO project_agent_classes (id, team_id, project_id, slug, name, status, handler_refs_json, metadata_json, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, params: [value.id, value.teamId, value.projectId, value.slug, value.name, value.status, JSON.stringify(value.handlerRefs), JSON.stringify(value.metadata ?? {}), now, now] },
			...operations,
		]);
	...
	async update(value: ProjectAgentClass, operations: CapacityDatabaseOperation[] = [], now = new Date().toISOString()): Promise<ProjectAgentClass> {
		await this.database.ensureInitialized();
		await this.database.batch([
			{ query: `UPDATE project_agent_classes SET slug = ?, name = ?, status = ?, handler_refs_json = ?, metadata_json = ?, updated_at = ? WHERE id = ? AND project_id = ?`, params: [value.slug, value.name, value.status, JSON.stringify(value.handlerRefs), JSON.stringify(value.metadata ?? {}), now, value.id, value.projectId] },
			...operations,
		]);
		return this.required(value.projectId, value.id);
	}
