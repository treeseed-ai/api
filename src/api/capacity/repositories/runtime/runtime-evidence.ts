...
function proxyHandle(row: JsonRecord | null): TreeDxProxyHandle | null {
	...
	return {
		...
		baseCommitSha: String(json(row.metadata_json, {}).baseCommitSha ?? '') || null,
		baseRef: String(json(row.metadata_json, {}).baseRef ?? '') || null,
		branchName: String(json(row.metadata_json, {}).branchName ?? '') || null,
		status: String(row.status),
