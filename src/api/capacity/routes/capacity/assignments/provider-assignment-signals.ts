...
import {
	assertProviderOwnsAssignment,assignmentActivityType,assignmentRecord,providerAssignmentErrorResponse,
	type ProviderAssignmentStore,
} from './provider-assignment-route-support.ts';
import { enforceProviderSignalContract,resolveSignalSubjectGroups } from './provider-signal-policy.ts';

function providerSignalInput(assignment: Record<string, unknown>, body: Record<string, unknown>, contract: AgentSignalContract, groupMembershipSnapshot: GroupMembershipSnapshot | null, groupMembershipSource: string) {
	...
	const declared = Array.isArray(assignmentRecord(assignment.allowedOutputs).publishedSignals)
		? assignmentRecord(assignment.allowedOutputs).publishedSignals as unknown[] : [];
...
export function installProviderAssignmentSignalRoutes(app: Hono, store: ProviderAssignmentStore) {
	app.post('/v1/provider/assignments/:assignmentId/signals', async (c) => {
		try {
			...
			const workdayRunId = assignmentRecord(assignment.metadata).workdayRunId;
			...
			try { parameters = assignmentRecord(JSON.parse(String(run.parameters_json ?? '{}'))); } catch { throw new CapacityGovernanceError('assignment_signal_workday_snapshot_invalid', 'The frozen workday parameters are corrupt.', 500); }
			const snapshot = decodeWorkdayPlanningGraphSnapshot(assignmentRecord(parameters.planningGraphByProjectId)[String(assignment.projectId)], String(assignment.projectId));
			...
			const groupMembershipSnapshot: GroupMembershipSnapshot | null = membership ? {
				...
				digest: typeof evidence.digest === 'string' ? evidence.digest : snapshot.revision, capturedAt: new Date().toISOString(),
			} : null;
			...
			if (contractId === 'proposal-participant-requested') {
				try { participantRequest = await validatePlanningParticipantRequest({ database: store, assignment, snapshot, payload: assignmentRecord(body.payload) }); }
				catch (error) {
					if (typeof workdayRunId === 'string' && store.createCapacityWorkdayEvent) await store.createCapacityWorkdayEvent(principal.teamId, workdayRunId, {
						...
						title: 'Planning participant request rejected', context: { contractId, target: assignmentRecord(body.payload), code: error instanceof CapacityGovernanceError ? error.code : 'planning_participant_request_failed', message: error instanceof Error ? error.message : String(error) },
					}).catch(() => null);
					throw error;
				}
			}
			...
			await store.run(`INSERT INTO agent_signals
				...
				signal.workdayRunId, assignment.id, assignment.agentId ?? null, activityType || null, principal.capacityProviderId,
				signal.causationId, signal.correlationId, signal.commitSha, signal.immutableRef, signal.digest,
				JSON.stringify(signal.changedPaths), signal.changeSummary, signal.evidenceRef, JSON.stringify(signal.payload), JSON.stringify(signal.metadata), now,
			]);
		...
		} catch (error) { return providerAssignmentErrorResponse(c, error); }
	});
}
