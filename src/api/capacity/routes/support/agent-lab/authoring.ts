...
async function draft(dependencies: WorkdayRouteDependencies, teamId: string, selectedProjectId?: string) {
	...
	const seed = { name: `agent-lab-${text(team?.slug, 'team')}`, version: 1, description: `Portable Agent Lab profile for ${text(team?.name, 'team')}.`, defaultEnvironments: ['local'], environments: ['local'], resources, runtime: { agentLabServicePrincipals: [{ key: `service-principal:${text(team?.slug)}/agent-lab`, environments: ['local'], team: teamKey, name: 'Agent Lab Service Principal', roles: ['team_owner'] }], capacityProviders: provider ? [{ key: `capacity-provider:${text(team?.slug)}/local`, providerClass: 'agent', environments: ['local'], team: teamKey, manifest: 'treeseed.capacity-provider.yaml', connectionId: 'primary-team', approval: 'trusted-local-owner', projects: [...projectKeys.values()], allowedModes: ['planning', 'acting'], executionProviderIds: ['codex'] }] : [] } };
