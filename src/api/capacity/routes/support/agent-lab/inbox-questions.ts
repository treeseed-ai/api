...
function body(source: string) { try { return parseFrontmatterDocument(source); } catch { return { frontmatter: {},body: source }; } }
...
export async function agentLabInboxQuestions(dependencies: WorkdayRouteDependencies,projects: Row[]) {
	const groups = await Promise.all(projects.map(async (project) => {
		...
		return (read.files ?? []).flatMap((candidate: unknown) => { const file = object(candidate); const path = text(file.path); const parsed = body(text(file.content)); const frontmatter = object(parsed.frontmatter); const status = text(frontmatter.status,'open').toLowerCase(); if (['answered','resolved','closed','complete','completed'].includes(status)) return [];
