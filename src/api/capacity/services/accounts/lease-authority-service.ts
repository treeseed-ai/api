...
export async function evaluateProviderAssignmentLeaseAuthority(
	...
	const [membership, reservation, workday, proxyHandle] = await Promise.all([
		...
		['workday_demand', 'living_execution_graph'].includes(String(assignment.synthesized_from))
	...
	if (!reservation) reasons.push('admission_reservation_missing');
		...
		if (!livingGraph && reservation.grant_status !== 'active') reasons.push('grant_not_active');
		if (!livingGraph && reservation.allocation_status !== 'active') reasons.push('allocation_not_active');
	}
	const leaseExpiry = assignment.lease_expires_at ? Date.parse(String(assignment.lease_expires_at)) : Number.NaN;
	const activeWorkdayStatus = livingGraph ? 'running' : 'active';
	...
	if (['workday_demand', 'living_execution_graph'].includes(String(assignment.synthesized_from)) && proxyHandle?.status !== 'issued') reasons.push('assignment_workspace_not_ready');
	...
	return {
		...
		gates: {
			assignmentAuthority: livingGraph ? 'living_execution_graph' : 'allocation',
