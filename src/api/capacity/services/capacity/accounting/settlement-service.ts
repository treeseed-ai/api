...
export async function settleCapacityReservationExactlyOnce(database: CapacityGovernanceDatabase, input: CapacitySettlementRequest) {
	...
	try {
		...
		await database.batch([
			...
			{ query: `INSERT INTO capacity_ledger_entries (id, settlement_key, membership_id, capacity_provider_id, reservation_id, assignment_id, mode_run_id, mode, team_id, project_id, work_day_id, task_id, phase, active_seconds, elapsed_seconds, provider_units, usd, source, metadata_json, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'overrun_hold', ?, ?, ?, ?, ?, ?, ?) ON CONFLICT (settlement_key) DO NOTHING`, params: [randomUUID(), holdKey, input.membershipId, reservation.capacity_provider_id, input.reservationId, input.assignmentId, input.modeRunId ?? null, reservation.mode ?? null, input.teamId, reservation.project_id ?? null, reservation.work_day_id ?? null, reservation.task_id ?? null, activeSeconds, elapsedSeconds, input.providerUnits ?? null, input.usd ?? null, input.source, JSON.stringify({ ...(input.metadata ?? {}), reservedSeconds, activeSeconds, cause: error instanceof Error ? error.message : String(error) }), now] },
