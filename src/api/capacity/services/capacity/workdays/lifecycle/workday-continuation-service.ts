...
export async function evaluateDurableWorkdayContinuation(
	...
	const totalSeconds = Number(envelope.availableSeconds);
	if (!Number.isFinite(totalSeconds) || totalSeconds < 0) throw new CapacityGovernanceError(
		'capacity_workday_time_budget_invalid', 'Workday continuation requires a non-negative finite agent-time budget.', 500,
		{ workdayId: input.workdayId, value: envelope.availableSeconds ?? null },
	);
	const totals = await database.first<{ committed_seconds?: unknown }>(
		`SELECT COALESCE(SUM(CASE WHEN state IN ('reserved','consuming') THEN reserved_seconds ELSE active_seconds END), 0) AS committed_seconds
		 FROM capacity_reservations WHERE team_id = ? AND work_day_id = ?`, [input.teamId, input.workdayId],
	);
	const committedSeconds = Number(totals?.committed_seconds ?? 0);
	if (!Number.isFinite(committedSeconds) || committedSeconds < 0) throw new CapacityGovernanceError('capacity_workday_committed_budget_invalid', 'Workday committed agent time is invalid.', 500, { workdayId: input.workdayId });
	...
	const decision = evaluateWorkdayContinuation({
		...
		totalSeconds, committedSeconds, usefulEligibleWork: input.usefulEligibleWork,
	});
	return { ...decision, totalSeconds, committedSeconds };
}
