import {
	...
	containsPlaintextSecretMaterial,
	validateSecretsCapabilityRegistry,
} from '@treeseed/sdk/secrets-capability';
import { reconcileTarget, type DesiredUnit } from '@treeseed/sdk/reconcile';
import { resolveGitHubCredentialForRepository } from '@treeseed/sdk/operations/github-credentials';
import { Octokit } from 'octokit';
import { createGitHubAppAdapter, resolveGitHubAppConfig } from '../reconciliation/github-app-adapter.ts';
...
export function createGitHubActionsSecretEnclave(options: {
	...
	workflowDispatcher?: (input: { token: string; unit: DesiredUnit; operation: any; dispatchRecord: any }) => Promise<any>;
	now?: () => Date;
}) {
	...
	async function deployEncryptedSecret(input: any) {
		...
		const deployment = assertGitHubActionsEncryptedSecretDeployment(input);
	...
	async function defaultWorkflowDispatcher(input: { token: string; unit: DesiredUnit }) {
