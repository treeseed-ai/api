...
const schema = z.object({ schemaVersion: z.literal('treeseed.identity-api-migration/v1'),
...
export async function migrateManagedApiIdentity(database: Parameters<typeof migrateLiveIdentity>[0], transport: typeof fetch = fetch) {
  ...
  try {
    ...
    const config = schema.parse(JSON.parse(descriptor.toString('utf8')));
  ...
  } catch { throw new Error('Managed Identity account migration failed; API writers must remain stopped'); }
