...
export function installOAuthProtocolRoutes(app: Hono, provider?: OAuthRuntimeProvider, authenticateBearer?: AuthenticateBearer,
	...
	app.get('/oauth/authorize', async (context) => {
		...
		return context.json({ schemaVersion: 'treeseed.oauth.authorization-presentation/v1', clientId,
			clientName: clientId === 'treeseed-admin' ? 'TreeSeed Admin' : 'TreeSeed CLI', redirectUri,
			redirectOrigin: new URL(redirectUri).origin, responseType: 'code', codeChallenge: challenge,
			codeChallengeMethod: 'S256', scopes, state: context.req.query('state') ?? '' }, 200,
	...
	app.post('/oauth/authorize', async (context) => {
		...
		if (String(body.decision ?? '') === 'deny') {
			const target = new URL(redirectUri);
			target.searchParams.set('error', 'access_denied');
			if (body.state) target.searchParams.set('state', String(body.state));
			return context.json({ schemaVersion: 'treeseed.oauth.authorization-decision/v1', approved: false,
				redirectTo: target.toString() }, 200, { 'cache-control': 'no-store', pragma: 'no-cache' });
		}
		if (String(body.decision ?? '') !== 'approve') return oauthError(context, 400, 'invalid_request', 'An explicit approve or deny decision is required.');
		...
		if (body.state) target.searchParams.set('state', String(body.state));
		return context.json({ schemaVersion: 'treeseed.oauth.authorization-decision/v1', approved: true,
			redirectTo: target.toString(), expiresIn: issued.expiresInSeconds },
			200, { 'cache-control': 'no-store', pragma: 'no-cache' });
	});
