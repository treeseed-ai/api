...
const headers = (authorization: string) => ({ accept: 'application/vnd.github+json', authorization,
	'user-agent': 'treeseed-provider-connector', 'x-github-api-version': '2022-11-28' });
...
export async function verifyGitHubInstallation(input: { appId: string; privateKey: string; installationId: string;
...
export async function exchangeGitHubUserCode(input: { clientId: string; clientSecret: string; code: string; fetchImpl?: typeof fetch }) {
	const response = await (input.fetchImpl ?? fetch)('https://github.com/login/oauth/access_token', { method: 'POST',
		headers: { accept: 'application/json', 'content-type': 'application/json', 'user-agent': 'treeseed-provider-connector' },
...
export async function verifyGitHubUserInstallation(input: { token: string; installationId: string; fetchImpl?: typeof fetch }) {
