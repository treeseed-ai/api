...
async function validateOperationCredentials(store: any, teamId: string, requests: HostedInfrastructureAuthorityRequest[]) {
  for (const request of requests) {
    ...
    if (!connection || connection.status !== 'active' || (request.credentialProfileId !== 'cloudflare-dns' && connection.nonSecretConfig?.deploymentEnvironment !== request.environment))
