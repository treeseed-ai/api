...
import { decodeWorkdayAgentProfileSnapshot } from '../../../capacity/services/capacity/workdays/policy/workday-agent-profile-policy.ts';
...
function signalInput(assignment: Record<string, unknown>, body: Record<string, unknown>, contract: AgentSignalContract,
	...
	return { id: `signal:${String(assignment.id)}:${contractId}:${idempotencyKey}`, contractId, subjectKind, subjectId, causationId, correlationId,
		...
		workdayRunId: assignmentWorkdayRunId(assignment) };
}
...
export function createProviderSignalService(store: ProviderAssignmentStore) {
	return async (auth: unknown, assignmentId: string, body: Record<string, unknown>) => {
		...
		const workdayRunId = assignmentWorkdayRunId(assignment);
		const run = workdayRunId ? await store.first('SELECT parameters_json FROM capacity_workday_runs WHERE id = ? AND team_id = ?', [workdayRunId, principal.teamId]) : null;
		...
		const snapshot = decodeWorkdayAgentProfileSnapshot(record(parameters.agentProfilesByProjectId)[String(assignment.projectId)], String(assignment.projectId));
