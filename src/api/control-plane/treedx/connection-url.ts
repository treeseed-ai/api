export function resolveTreeDxServiceUrl(value: string, environment: Record<string, unknown>): string {
	...
	const authority = /^([a-z0-9.-]+):(\d{1,5})$/iu.exec(candidate);
	const normalized = local && authority && localHttpHosts.has(authority[1]!)
		? `http://${candidate}`
		: candidate;
	let parsed: URL;
	try { parsed = new URL(normalized); } catch { throw new Error('TreeDX connection URL is invalid.'); }
	if (!['http:', 'https:'].includes(parsed.protocol) || parsed.username || parsed.password || parsed.search || parsed.hash) {
		const violations = [
			!['http:', 'https:'].includes(parsed.protocol) ? 'protocol' : '',
			parsed.username || parsed.password ? 'embedded-credentials' : '',
			parsed.search ? 'query' : '',
			parsed.hash ? 'fragment' : '',
		].filter(Boolean).join(',');
		throw new Error(`TreeDX connection requires a credential-free HTTP service URL without query or fragment data (invalid: ${violations}; protocol: ${parsed.protocol || 'missing'}).`);
	}
