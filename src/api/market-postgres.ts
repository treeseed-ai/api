...
const postgresPoolErrorLogger = Symbol.for('treeseed.postgresPoolErrorLogger');

function attachPostgresPoolErrorLogger(pool) {
	if (!pool || typeof pool.on !== 'function' || pool[postgresPoolErrorLogger]) return;
	pool[postgresPoolErrorLogger] = true;
	pool.on('error', (error) => {
