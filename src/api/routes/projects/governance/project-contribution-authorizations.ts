...
export function installProjectContributionAuthorizationRoutes(context:any) {
	...
	const signingSecret=()=>String(store.config?.contributionSigningSecret??'').trim();
	const requireSigningSecret=(c:any)=>signingSecret()?null:jsonError(c,503,'Project contribution receipt signing is not configured.',{code:'contribution_signing_key_unavailable'});
	...
	app.get('/v1/projects/:projectId/contribution-authorizations',async(c:any)=>{const allowed=await access(c);if(allowed.response)return allowed.response;return c.json({ok:true,payload:{items:await listProjectContributionAuthorizations(store,c.req.param('projectId')),grantProfiles:{apache:projectContributionGrant({name:'sdk'}),dualLicense:projectContributionGrant({name:'api'})}}});});
	app.post('/v1/projects/:projectId/contribution-authorizations/plan',async(c:any)=>{const allowed=await owner(c);if(allowed.response)return allowed.response;const unavailable=requireSigningSecret(c);if(unavailable)return unavailable;const body=await c.req.json().catch(()=>({}));return c.json({ok:true,payload:planProjectContributionAuthorization({...body,projectId:c.req.param('projectId')},allowed.principal,signingSecret())});});
	app.post('/v1/projects/:projectId/contribution-authorizations/apply',async(c:any)=>{const allowed=await owner(c);if(allowed.response)return allowed.response;const unavailable=requireSigningSecret(c);if(unavailable)return unavailable;const body=await c.req.json().catch(()=>({}));const result=await applyProjectContributionAuthorization(store,{...body,projectId:c.req.param('projectId')},allowed.principal,signingSecret());return c.json(result,result.ok?201:result.code==='contribution_authorization_confirmation_mismatch'?409:400);});
