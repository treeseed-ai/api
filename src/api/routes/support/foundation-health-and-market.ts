export function installFoundationHealthAndMarketRoutes(context: any) {
	const { app, bearerTokenFromRequest, capacity, config, installCapacityRoutes, requireProjectAccess, requireTeamAccess, runtime, runtimeMarketAuthProvider, sessionEvents, store } = context;
	...
	installCapacityRoutes(app, { store: capacity, requireTeamAccess, requireProjectAccess, runtime, runtimeMarketAuthProvider, sessionEvents, config: { ...config, ...runtime.resolved.config } });
}
