export function installFoundationHealthAndMarketRoutes(context: any) {
	const { app, bearerTokenFromRequest, capacity, config, installCapacityRoutes, localAcceptanceAdminToken, localAcceptanceAuthEnabled, requireProjectAccess, requireTeamAccess, runtime, runtimeMarketAuthProvider, store } = context;
