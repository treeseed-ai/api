...
function normalizedCapabilityStrings(values) {
	const collected = [];
	for (const value of arrayValue(values)) {
		...
		const record = value;
		const id = stringValue(record.id ?? record.name ?? record.capability);
		if (id) collected.push(id);
		for (const alias of arrayValue(record.aliases ?? record.capabilityAliases ?? record.metadata?.capabilityAliases)) {
...
function sessionCapabilities(session, provider) {
	return [...new Set([
		...normalizedCapabilityStrings(provider?.capabilities),
		...normalizedCapabilityStrings(session?.capabilities),
		...normalizedCapabilityStrings(session?.metadata?.capabilities),
	])];
}
...
function collectContentArtifactRefsFromExecution(row) {
	const refs = [];
	const modeOutputs = parseJson(row.outputs_json, {});
	const assignmentOutput = parseJson(row.assignment_lifecycle_output_json ?? row.lifecycle_output_json, {});
	for (const candidate of [
		modeOutputs?.metadata?.contentArtifactRefs,
		modeOutputs?.metadata?.artifact?.items,
		assignmentOutput?.metadata?.contentArtifactRefs,
		assignmentOutput?.metadata?.artifact?.items,
	]) {
		if (Array.isArray(candidate)) refs.push(...candidate.filter((entry) => entry && typeof entry === 'object'));
	}
	return refs;
}
...
function tokenDiagnosticsFromExecution(row) {
	const usage = parseJson(row.usage_actual_json, {});
	const outputs = parseJson(row.outputs_json, {});
	const snapshot = outputs?.metadata?.executionSnapshot ?? outputs?.metadata?.artifact?.executionSnapshot ?? {};
	const codex = snapshot?.metadata?.codex ?? outputs?.metadata?.codex ?? {};
	const rawUsage = codex?.usage ?? snapshot?.usage ?? usage?.nativeUsage?.executionUsage ?? null;
	return {
		inputTokens: Number(rawUsage?.input_tokens ?? rawUsage?.inputTokens ?? rawUsage?.prompt_tokens ?? rawUsage?.promptTokens ?? NaN) || null,
		outputTokens: Number(rawUsage?.output_tokens ?? rawUsage?.outputTokens ?? rawUsage?.completion_tokens ?? rawUsage?.completionTokens ?? NaN) || null,
		cachedInputTokens: Number(rawUsage?.cached_input_tokens ?? rawUsage?.cachedInputTokens ?? NaN) || null,
...
function serializeCommerceCapacityListing(row, options = {}) {
	...
	const listing = {
		...
		sellerTeamId: row.seller_team_id,
		capacityProviderId: row.capacity_provider_id,
		capacityProviderLaneId: row.capacity_provider_lane_id,
		status: row.status,
...
function serializeWorkdayTestRun(row) {
	if (!row) return null;
	return {
		id: row.id,
		teamId: row.team_id,
		...
		actual: parseJson(row.actual_json, {}),
		reportRefs: parseJson(row.report_refs_json, {}),
...
function serializeWorkdayTestEvent(row) {
	if (!row) return null;
	return {
		id: row.id,
		runId: row.run_id,
...
export class MarketControlPlaneStore {
	...
	async recordProviderCheckIn(principal, input = {}) {
		...
		const nowMs = Date.parse(now);
		const serializedRows = rows.map(serializeProviderAssignment).filter(Boolean);
		const workDayIds = [...new Set(serializedRows.map((assignment) => assignment.workDayId ?? assignment.capacityEnvelope?.workDayId ?? null).filter(Boolean))];
		...
		const workdaySortWeight = (assignment) => {
		...
		const leaseRetryWeight = (assignment) => {
			if (assignment.status === 'pending' && assignment.leaseState === 'unleased') return 0;
			if (assignment.leaseState === 'leased' && assignment.leaseExpiresAt) return 1;
			if (assignment.status === 'returned' && assignment.leaseState === 'released') return 2;
			return 3;
		};
		const leaseGateForAssignment = (assignment) => {
			...
			if (assignment.leaseState === 'leased' && assignment.leaseExpiresAt) {
				...
				if (Number.isFinite(expiresAt) && expiresAt <= nowMs) return { leasable: true, reasons: [] };
				return { leasable: false, reasons: ['lease_still_active'] };
			}
			const reasons = [];
			if (!['pending', 'returned', 'leased'].includes(String(assignment.status ?? ''))) reasons.push('status_not_leasable');
			if (!['unleased', 'released', 'expired', 'leased'].includes(String(assignment.leaseState ?? ''))) reasons.push('lease_state_not_leasable');
			if (!reasons.length) reasons.push('lease_state_not_ready');
			return { leasable: false, reasons };
		};
		const candidateDiagnostics = [];
		const leasable = serializedRows
			.filter((assignment) => {
				const gate = leaseGateForAssignment(assignment);
				if (!gate.leasable) {
			...
			.sort((left, right) =>
				workdaySortWeight(left) - workdaySortWeight(right)
				|| leaseRetryWeight(left) - leaseRetryWeight(right)
		...
		for (const candidate of leasable) {
			...
			if (eligibility.eligible) {
				candidateDiagnostics.push({
					...
					sessionId: context.session?.id ?? candidate.providerSessionId ?? null,
					reasons: [],
					gates: eligibility.gates,
					matchingGrantIds: eligibility.gates?.matchingGrantIds ?? [],
					checkedInGrantIds: eligibility.gates?.checkedInGrantIds ?? [],
					missingCapabilities: eligibility.gates?.missingCapabilities ?? [],
					workdayStatus: eligibility.gates?.workDayStatus ?? null,
					runnerPressure: eligibility.gates?.runnerPressure ?? null,
				...
				await this.recordProviderAssignmentExplanation(principal.teamId, candidate.id, {
					...
					reasons: eligibility.reasons,
					gates: eligibility.gates,
					metadata: { evaluatedAt: now, diagnosticsSource: 'provider_lease_attempt' },
				}).catch(() => null);
			}
		}
		...
		const diagnostics = {
			...
			totals: {
				candidates: serializedRows.length,
				leasable: leasable.length,
				selected: assignment ? 1 : 0,
				skipped: Math.max(0, candidateDiagnostics.filter((candidate) => !candidate.selected).length),
			},
		...
		if (!assignment) {
			...
			return { assignment: null, leaseToken: null, leaseSeconds, diagnostics };
		}
	...
	async listExecutionRunsForTeam(teamId, filters = {}) {
		await this.ensureInitialized();
		...
		if (filters.executionProviderId) {
			clauses.push('COALESCE(m.execution_provider_id, a.execution_provider_id) = ?');
		...
		const rows = await this.all(
			`SELECT
			 ...
			 JOIN provider_assignments a ON a.id = m.provider_assignment_id
			...
			return rows.map((row) => {
				...
				input: {
					...
					decisionInput: parseJson(row.assignment_decision_input_json, {}),
					capacityEnvelope: run.capacityEnvelope,
					workspaceContext: parseJson(row.assignment_workspace_context_json, {}),
					allowedOutputs: parseJson(row.assignment_allowed_outputs_json, {}),
				},
				output: {
					outputs: run.outputs,
					lifecycleOutput: parseJson(row.assignment_lifecycle_output_json, {}),
	...
	async validateCommerceCapacityProviderDisclosure(product, input = {}) {
		if (!input.capacityProviderId && !input.capacityProviderLaneId) return;
		if (!input.capacityProviderId) {
			const error = new Error('capacityProviderId is required when linking a capacity provider lane.');
			error.status = 400;
			throw error;
		}
		const provider = await this.getCapacityProvider(product.sellerTeamId, input.capacityProviderId);
		if (!provider && !input.marketAdmin) {
			const error = new Error('Capacity provider must be controlled by the seller team.');
			error.status = 403;
			throw error;
		}
		if (input.capacityProviderLaneId) {
			const lane = await this.first(`SELECT * FROM capacity_provider_lanes WHERE id = ? LIMIT 1`, [input.capacityProviderLaneId]);
			if (!lane || lane.capacity_provider_id !== input.capacityProviderId) {
				const error = new Error('Capacity provider lane must belong to the linked provider.');
				error.status = 400;
				throw error;
			}
		}
	...
	async ensureCommerceCapacityListingEligibility(product, input = {}) {
		if (!product) {
		...
		await this.validateCommerceCapacityProviderDisclosure(product, input);
		return vendor;
	}

	async createCommerceCapacityListing(productId, input = {}) {
		await this.ensureInitialized();
		const product = await this.getCommerceProduct(productId);
		const vendor = await this.ensureCommerceCapacityListingEligibility(product, input);
		...
		await this.run(
			`INSERT INTO commerce_capacity_listings (
				id, product_id, vendor_id, seller_team_id, capacity_provider_id, capacity_provider_lane_id, status,
			...
			[
				...
				input.capacityProviderId ?? null,
				input.capacityProviderLaneId ?? null,
				'draft',
		...
		await this.recordCommerceGovernanceEvent({
			...
			evidence: input.evidence ?? {
				...
				capacityProviderLaneId: input.capacityProviderLaneId ?? null,
			},
	...
	async updateCommerceCapacityListing(listingId, input = {}) {
		await this.ensureInitialized();
		...
		await this.ensureCommerceCapacityListingEligibility(product, {
			...
			capacityProviderLaneId: input.capacityProviderLaneId === undefined ? existing.capacityProviderLaneId : input.capacityProviderLaneId,
		});
		const timestamp = isoNow();
		await this.run(
			`UPDATE commerce_capacity_listings
			 SET capacity_provider_id = ?, capacity_provider_lane_id = ?, access_level = ?, runtime_isolation_level = ?,
			...
			[
				...
				input.capacityProviderLaneId === undefined ? existing.capacityProviderLaneId : input.capacityProviderLaneId,
	...
	async transitionCommerceCapacityListing(listingId, nextState, input = {}) {
		await this.ensureInitialized();
		const existing = await this.getCommerceCapacityListing(listingId);
		if (!existing) return null;
		const product = await this.getCommerceProduct(existing.productId);
		await this.ensureCommerceCapacityListingEligibility(product, input);
	...
	async submitCommerceCapacityListing(listingId, input = {}) {
		return this.transitionCommerceCapacityListing(listingId, 'submitted', input);
	}

	async approveCommerceCapacityListing(listingId, input = {}) {
		return this.transitionCommerceCapacityListing(listingId, 'approved', { ...input, marketAdmin: true });
	}

	async rejectCommerceCapacityListing(listingId, input = {}) {
		return this.transitionCommerceCapacityListing(listingId, 'rejected', { ...input, marketAdmin: true });
	}

	async suspendCommerceCapacityListing(listingId, input = {}) {
		return this.transitionCommerceCapacityListing(listingId, 'suspended', { ...input, marketAdmin: true });
	}

	async archiveCommerceCapacityListing(listingId, input = {}) {
		return this.transitionCommerceCapacityListing(listingId, 'archived', input);
	}
	...
	async createWorkdayTestRun(teamId, input = {}) {
		await this.ensureInitialized();
		const timestamp = isoNow();
		const id = input.id ?? randomUUID();
		...
		await this.run(
			`INSERT INTO workday_test_runs (
				...
				parameters_json, summary_json, metrics_json, expected_json, actual_json, report_refs_json, error_json,
	...
	async updateWorkdayTestRun(teamId, runId, input = {}) {
		await this.ensureInitialized();
		const existing = await this.getWorkdayTestRun(teamId, runId);
		if (!existing) return null;
		const timestamp = isoNow();
		const status = stringValue(input.status, existing.status);
		await this.run(
			`UPDATE workday_test_runs
			 SET capacity_provider_id = ?, scenario_id = ?, status = ?, environment = ?,
			     parameters_json = ?, summary_json = ?, metrics_json = ?, expected_json = ?, actual_json = ?,
			     report_refs_json = ?, error_json = ?, started_at = ?, completed_at = ?, updated_at = ?
			 WHERE id = ? AND team_id = ?`,
			[
				...
				JSON.stringify(objectValue(input.expected ?? existing.expected)),
				JSON.stringify(objectValue(input.actual ?? existing.actual)),
				JSON.stringify(objectValue(input.reportRefs ?? input.report_refs ?? existing.reportRefs)),
				JSON.stringify(objectValue(input.error ?? existing.error)),
				input.startedAt ?? existing.startedAt ?? (status === 'running' ? timestamp : null),
				input.completedAt ?? existing.completedAt ?? (['completed', 'failed', 'degraded'].includes(status) ? timestamp : null),
				timestamp,
				runId,
				teamId,
			],
	...
	async listWorkdayTestRuns(teamId, filters = {}) {
		...
		const rows = await this.all(
			`SELECT * FROM workday_test_runs WHERE ${clauses.join(' AND ')} ORDER BY created_at DESC LIMIT 100`,
			values,
		);
		return rows.map(serializeWorkdayTestRun);
	}

	async createWorkdayTestEvent(teamId, runId, input = {}) {
		await this.ensureInitialized();
		const run = await this.getWorkdayTestRun(teamId, runId);
		if (!run) return null;
		const timestamp = isoNow();
		const row = await this.first(
			`SELECT COALESCE(MAX(event_index), -1) AS max_index FROM workday_test_events WHERE run_id = ? AND team_id = ?`,
			[runId, teamId],
		);
		const eventIndex = Number.isFinite(Number(input.eventIndex)) ? Number(input.eventIndex) : Number(row?.max_index ?? -1) + 1;
		const id = input.id ?? `${runId}-event-${String(eventIndex).padStart(4, '0')}`;
		await this.run(
			`INSERT OR REPLACE INTO workday_test_events (
				id, run_id, team_id, project_id, workday_id, assignment_id, mode_run_id, event_index,
				event_type, status, title, message, parameters_json, context_json, refs_json, metadata_json, created_at
			) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
			[
				id,
				runId,
				teamId,
				input.projectId ?? null,
				input.workdayId ?? input.workDayId ?? null,
				input.assignmentId ?? null,
				input.modeRunId ?? null,
				eventIndex,
				stringValue(input.eventType, input.type, 'event'),
				stringValue(input.status, 'recorded'),
				input.title ?? null,
				input.message ?? null,
				JSON.stringify(objectValue(input.parameters)),
				JSON.stringify(objectValue(input.context)),
				JSON.stringify(objectValue(input.refs)),
				JSON.stringify(objectValue(input.metadata)),
				input.createdAt ?? timestamp,
			],
		);
		return serializeWorkdayTestEvent(await this.first(`SELECT * FROM workday_test_events WHERE id = ? LIMIT 1`, [id]));
	}

	async listWorkdayTestEvents(teamId, runId) {
		await this.ensureInitialized();
		const rows = await this.all(
			`SELECT * FROM workday_test_events WHERE team_id = ? AND run_id = ? ORDER BY event_index ASC, created_at ASC LIMIT 2000`,
			[teamId, runId],
		);
		return rows.map(serializeWorkdayTestEvent);
	}
