...
export async function createSecretMetadataRecordMethod(this: MarketControlPlaneStore, input: any = {}) {
    ...
    const problems = validateWritableSecretMetadata(record);
    if (problems.length > 0)
        throw secretCapabilityValidationError(problems, 'Invalid secret metadata record.');
    await this.run(`INSERT INTO secret_metadata_records (
