...
export async function heartbeatKnowledgeWorkspaceMethod(this: MarketControlPlaneStore, workspaceId: string, userId: string) {
...
export async function listKnowledgeWorkspacePresenceMethod(this: MarketControlPlaneStore, workspaceId: string) {
	await this.ensureInitialized();
...
export async function createKnowledgeReviewCommentMethod(this: MarketControlPlaneStore, input: any) {
	await this.ensureInitialized();
...
export async function listKnowledgeReviewCommentsMethod(this: MarketControlPlaneStore, reviewId: string) {
	await this.ensureInitialized();
	return (await this.all(`SELECT * FROM knowledge_review_comments WHERE review_id = ? ORDER BY created_at ASC`, [reviewId])).map(comment);
}

export async function resolveKnowledgeReviewCommentMethod(this: MarketControlPlaneStore, id: string, input: any) {
	await this.ensureInitialized();
