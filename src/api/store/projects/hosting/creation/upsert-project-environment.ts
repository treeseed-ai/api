...
export async function upsertProjectEnvironmentMethod(this: MarketControlPlaneStore, projectId, input) {
    await this.ensureInitialized();
    const timestamp = isoNow();
    const existing = await this.first(`SELECT * FROM project_environments WHERE project_id = ? AND environment = ? LIMIT 1`, [projectId, input.environment]);
    if (existing) {
        await this.run(`UPDATE project_environments
        ...
        await this.run(`INSERT INTO project_environments (
    ...
    return serializeProjectEnvironment(await this.first(`SELECT * FROM project_environments WHERE project_id = ? AND environment = ? LIMIT 1`, [projectId, input.environment]));
}
