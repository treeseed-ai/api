...
export async function createClientEncryptedEscrowRecordMethod(this: MarketControlPlaneStore, input: any = {}) {
    ...
    const problems = validateClientEncryptedEscrowMetadata(record);
    if (problems.length > 0)
        throw secretCapabilityValidationError(problems, 'Invalid client-encrypted escrow record.');
    await this.run(`INSERT INTO client_encrypted_escrow_records (
