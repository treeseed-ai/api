...
export async function updateClientEncryptedEscrowRecordMethod(this: MarketControlPlaneStore, id, patch: any = {}) {
    ...
    const problems = validateClientEncryptedEscrowMetadata(next);
    if (problems.length > 0)
        throw secretCapabilityValidationError(problems, 'Invalid client-encrypted escrow update.');
    const timestamp = isoNow();
    await this.run(`UPDATE client_encrypted_escrow_records
