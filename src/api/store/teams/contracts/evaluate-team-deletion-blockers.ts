...
export async function evaluateTeamDeletionBlockersMethod(this: MarketControlPlaneStore, teamId) {
    ...
    const [
        ...
        this.all(`SELECT id, provider, name, status FROM team_web_hosts
                 WHERE team_id = ? AND status <> 'deleted' ORDER BY created_at ASC LIMIT 20`, [teamId]),
        this.all(`SELECT id, provider, name, status FROM repository_hosts
                 WHERE team_id = ? AND status <> 'deleted' ORDER BY created_at ASC LIMIT 20`, [teamId]),
        this.all(`SELECT id, project_id, environment, status FROM project_deployments
    ...
    const activeDeployments = deployments.filter((row) => !PROJECT_DEPLOYMENT_TERMINAL_STATUSES.includes(String(row.status)));
    return [
        ...projects.map((row) => ({ code: 'project', id: row.id, label: row.name, href: `/app/projects/${row.id}/settings` })),
        ...webHosts.map((row) => ({ code: 'host', id: row.id, label: row.name, href: '/app/hosts' })),
        ...repositoryHosts.map((row) => ({ code: 'repository_host', id: row.id, label: row.name, href: '/app/hosts' })),
        ...activeDeployments.map((row) => ({ code: 'active_deployment', id: row.id, label: `${row.environment} deployment`, href: `/app/projects/${row.project_id}/deployments` })),
