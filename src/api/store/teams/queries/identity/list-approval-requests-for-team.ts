...
export async function listApprovalRequestsForTeamMethod(this: ControlPlaneStore, teamId, options: any = {}) {
	...
	const rows = kind
		? await this.all(`SELECT * FROM approval_requests WHERE team_id = ? AND kind = ? ORDER BY created_at DESC LIMIT ?`, [teamId, kind, limit])
		: await this.all(`SELECT * FROM approval_requests WHERE team_id = ? ORDER BY created_at DESC LIMIT ?`, [teamId, limit]);
