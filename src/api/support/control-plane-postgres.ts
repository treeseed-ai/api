...
function resolveControlPlaneMigrationRoot() {
	const candidates = [
		resolve(process.cwd(), 'drizzle/control-plane'),
		resolve(process.cwd(), 'packages/api/drizzle/control-plane'),
		resolve(dirname(fileURLToPath(import.meta.url)), '../../../drizzle/control-plane'),
	];
	...
	if (!found) {
		throw new Error('Unable to locate the API control-plane migration baseline. Ensure drizzle/control-plane/0000_control_plane.sql is included in the API package.');
	}
	return found;
}

class ControlPlanePostgresPreparedStatement {
	private bindings: unknown[] = [];

	constructor(
		private readonly database: ControlPlanePostgresDatabase,
...
export class ControlPlanePostgresDatabase {
	pool: Pool;
	...
	prepare(query: string): ControlPlanePostgresPreparedStatement {
		return new ControlPlanePostgresPreparedStatement(this, query);
	}
	...
	private async applyDrizzleMigrations(): Promise<void> {
		...
		if (files.length === 0) {
			throw new Error(`No API control-plane migration SQL files found in ${migrationRoot}.`);
		}
		if (!(await tableExists(this.pool, 'treeseed_control_plane_schema_migrations'))) {
			await this.pool.query(`CREATE TABLE IF NOT EXISTS treeseed_control_plane_schema_migrations (
				name text PRIMARY KEY,
				applied_at text NOT NULL
			)`);
		}
		const applied = await this.pool.query(`SELECT name FROM treeseed_control_plane_schema_migrations`);
		const appliedNames = new Set(applied.rows.map((row) => row.name));
		if (appliedNames.has('0000_control_plane.sql') && !(await hasAdoptableBaselineSchema(this.pool))) {
			appliedNames.delete('0000_control_plane.sql');
		}
		for (const file of files) {
			if (appliedNames.has(file)) continue;
			if (file === '0000_control_plane.sql' && await hasAdoptableBaselineSchema(this.pool)) {
				await this.pool.query(
					`INSERT INTO treeseed_control_plane_schema_migrations (name, applied_at)
					 VALUES ($1, $2)
			...
			try {
				...
				for (const statement of splitSqlList(sql)) {
					...
					const statementToApply = createIndex
						...
						: statement;
					await client.query(translateControlPlaneSqlToPostgres(statementToApply));
				}
				await client.query(
					`INSERT INTO treeseed_control_plane_schema_migrations (name, applied_at)
					 VALUES ($1, $2)
				...
				await client.query('ROLLBACK');
				throw new Error(`Failed to apply API control-plane migration ${file}: ${error instanceof Error ? error.message : String(error)}`);
			} finally {
				client.release();
			}
		}
