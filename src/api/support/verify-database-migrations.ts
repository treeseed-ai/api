...
/** Application startup never creates/adopts tables or applies pending migrations. */
export async function verifyDatabaseMigrations(pool: {query: (sql: string) => Promise<{rows: any[]}>}, migrationRoot: string) {
  const files = readdirSync(migrationRoot).filter(file => file.endsWith('.sql')).sort();
  if (!files.length) throw new Error('database_migration_inventory_missing');
  const exists = await pool.query("SELECT to_regclass('public.treeseed_control_plane_schema_migrations') AS ledger");
  if (!exists.rows[0]?.ledger) throw new Error('database_requires_explicit_migration');
