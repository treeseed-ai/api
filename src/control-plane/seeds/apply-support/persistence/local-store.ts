...
export async function createLocalSeedStore(projectRoot, env = process.env) {
    ...
    const db = createControlPlanePostgresDatabase(apiDatabaseUrl);
    return new ControlPlaneStore({
        repoRoot: projectRoot,
        projectId: localEnv.TREESEED_PROJECT_ID ?? 'treeseed-api',
		...
		assertionSecret: localEnv.TREESEED_API_WEB_ASSERTION_SECRET ?? localEnv.TREESEED_WEB_ASSERTION_SECRET ?? 'treeseed-local-seed-assertion-secret',
