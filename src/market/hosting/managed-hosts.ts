...
async function collectLocalConfigValues(runtime, scope = 'prod') {
	...
	try {
		...
		const { collectConfigSeedValues } = await import(/* @vite-ignore */ operationsSpecifier);
		if (typeof collectConfigSeedValues === 'function') {
			return collectConfigSeedValues(process.cwd(), scope, runtime?.env ?? process.env);
		}
	} catch {
		return {};
	}
...
export function resolveManagedCloudflareHostConfig(runtime, values: any = {}) {
	...
	const config = {
		...
		CLOUDFLARE_PAGES_PROJECT_NAME: firstRuntimeEnvValue(runtime, ['TREESEED_CLOUDFLARE_PAGES_PROJECT_NAME'], values),
		CLOUDFLARE_PAGES_PREVIEW_PROJECT_NAME: firstRuntimeEnvValue(runtime, ['TREESEED_CLOUDFLARE_PAGES_PREVIEW_PROJECT_NAME'], values),
		...
		CONTENT_BUCKET_NAME: firstRuntimeEnvValue(runtime, ['TREESEED_CONTENT_BUCKET_NAME'], values),
		CONTENT_BUCKET_BINDING: firstRuntimeEnvValue(runtime, ['TREESEED_CONTENT_BUCKET_BINDING'], values),
	};
	return Object.fromEntries(Object.entries(config).filter(([, value]) => value));
}

export async function resolveManagedCloudflareHostConfigFromConfig(runtime, scope = 'prod') {
	return resolveManagedCloudflareHostConfig(runtime, await collectLocalConfigValues(runtime, scope));
}
...
export function listManagedHosts(teamId, runtime, values: any = {}) {
	const cloudflareConfig = resolveManagedCloudflareHostConfig(runtime, values);
...
export async function listManagedHostsFromConfig(teamId, runtime, scope = runtime?.resolved?.config?.environment ?? 'prod') {
	return listManagedHosts(teamId, runtime, await collectLocalConfigValues(runtime, scope));
}
