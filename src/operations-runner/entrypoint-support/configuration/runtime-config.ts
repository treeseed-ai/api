...
export async function loadConfig({ requireSecrets = true }: any = {}) {
    ...
    const config = {
        serverUrl: env('TREESEED_API_BASE_URL') ?? env('TREESEED_URL'),
        apiDatabaseUrl,
        serverId,
        runnerId: env('TREESEED_PLATFORM_RUNNER_ID', serverId === 'prod' ? 'treeseed-ops-prod-1' : serverId === 'staging' ? 'treeseed-ops-staging-1' : 'treeseed-ops-local-1'),
        ...
        environment: env('TREESEED_PLATFORM_RUNNER_ENVIRONMENT', serverId === 'prod' ? 'production' : serverId),
    ...
    if (requireSecrets) {
        const missing = config.apiDatabaseUrl
            ? []
            : Object.entries({
                TREESEED_API_BASE_URL: config.serverUrl,
