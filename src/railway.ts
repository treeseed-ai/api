...
async function honoNodeHandler(app: Hono, request: Parameters<Server['emit']>[1], response: Parameters<Server['emit']>[2]) {
...
export async function createRailwayTreeseedApiServer(options: ApiServerOptions = {}) {
	const config = {
		...resolveApiConfig(),
		...(options.config ?? {}),
		providers: {
