...
describe('API deploy workflow', () => {
	...
	it('owns API reconciliation, live verification, runner smoke, and acceptance', () => {
		...
		expect(liveRun).toContain('Resolve Treeseed CLI binary');
		expect(liveRun).toContain('node_modules/@treeseed/cli/package.json');
		expect(liveRun).toContain("bin.includes('/src/')");
		expect(liveRun).toContain('node \\"${cli_bin}\\" --help >/dev/null');
		expect(liveRun).toContain('node \\"${TREESEED_CLI_BIN}\\" secrets:unlock');
		expect(liveRun).not.toContain('./node_modules/.bin/trsd');
		...
		expect(liveRun).toContain('node \\"${TREESEED_CLI_BIN}\\" hosting verify');
		...
		expect(liveRun).toContain('node \\"${TREESEED_CLI_BIN}\\" operations smoke');
