...
import { DerivedCapacityService, summarizeDerivedCapacity } from '../../src/api/capacity/services/derived-capacity-service.ts';
...
describe('derived capacity service', () => {
	...
	it('returns no capacity for a suspended membership even when provider inventory is supplied', async () => {
		const all = vi.fn();
		const database = {
			ensureInitialized: vi.fn(), run: vi.fn(), batch: vi.fn(), all,
			first: vi.fn().mockResolvedValue(membershipRow({ membership_status: 'suspended' })),
		};
	...
	it('excludes revoked identities from team capacity before loading provider inventory', async () => {
		...
		const service = new DerivedCapacityService({ ensureInitialized: vi.fn(), run: vi.fn(), batch: vi.fn(), all, first });
