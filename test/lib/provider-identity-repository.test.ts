...
import { CapacityProviderIdentityRepository, serializeCapacityProviderMembershipView } from '../../src/api/capacity/repositories/provider-identity.ts';
...
describe('capacity provider identity repository', () => {
	...
	it('keeps every query team-scoped and preserves storage failures', async () => {
		...
		const database = {
			...
			all: async (query: string, params?: unknown[]) => { calls.push({ query, params }); return [row()]; },
			first: async (query: string, params?: unknown[]) => { calls.push({ query, params }); return row(); },
		};
