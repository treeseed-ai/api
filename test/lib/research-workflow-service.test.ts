...
describe('durable research workflow service', () => {
	it('persists and CAS-advances rejection, revision, publication, and reporting', async () => {
		...
		try {
			...
			expect(await store.createResearchWorkflow('project-a', { id: 'other', objectiveRef: 'objective:a', questionRef: 'question:a', idempotencyKey: 'research:a' })).toEqual(workflow);
	...
	it('fails closed for stale state and invalid ordering without mutating the workflow', async () => {
		...
		try {
			...
			await expect(store.completeResearchWorkflowStage(workflow!.id, 'governed-source-search', { expectedStateVersion: 1, assignmentId: 'a', artifactRefs: ['a'] })).rejects.toMatchObject({ code: 'research_workflow_stage_not_ready' });
	...
	it('projects only the ready workflow stage to its configured research role', async () => {
		...
		try {
			...
			await expect(resolvePlanningDemandSource(store, run, { id: 'project-a' }, researcher, intent)).resolves.toMatchObject({
				...
				payload: { researchWorkflowId: workflow!.id, researchWorkflowStateVersion: 1, researchStage: 'question-decomposition', questionRef: 'question:a', intent: { artifactKind: 'planning_question', subjectModel: 'question' } },
			});
		} finally { await database.close(); }
	});
