...
import { serializeStructuredAgentEstimateRow } from '../../src/api/capacity/repositories/structured-estimate.ts';
import { installStructuredEstimateRoutes } from '../../src/api/capacity/routes/structured-estimates.ts';
...
describe('structured agent estimate service', () => {
	...
	it('enforces one-way status transitions and strict status filters', async () => {
		...
		try {
			...
			await expect(store.listStructuredAgentEstimatesForDecision('decision-a', { status: 'invented' })).rejects.toMatchObject({ code: 'structured_agent_estimate_status_invalid' });
		} finally { await database.close(); }
	});
