...
import { authorizeTreeDxProxy } from '../../src/api/capacity/services/treedx-proxy-access-service.ts';
import { treeDxTokenScope } from '../../src/api/capacity/services/treedx-proxy-token-service.ts';
...
function app(store: Record<string, unknown>) {
	...
	application.get('/v1/dx/projects/:projectId/workspaces/:workspaceId/files', async (c) => {
		try {
			...
			if (error?.status) return c.json({ ok: false, code: error.code }, { status: error.status });
			throw error;
		}
	});
	return application;
