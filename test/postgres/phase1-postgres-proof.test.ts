...
describe.skipIf(!sourceUrl)('Phase 1 real PostgreSQL proof', () => {
	...
	it('serializes workday events and enforces run, allocation, and graph concurrency through the domain owners', async () => {
		...
		try {
			...
			const runStore = Object.create(store) as MarketControlPlaneStore;
			runStore.scheduleCapacityWorkdayRun = async () => ({ scheduled: true });
			runStore.terminalizeCapacityWorkdayAssignments = async () => ({ terminalized: true });
			runStore.terminalizeCapacityWorkdayEnvelopes = async () => ({ terminalized: true });
