...
import { aggregateCapacityTimeReservations } from '../../../../../src/api/capacity/services/capacity/accounting/agent-time-reservation-aggregation-service.ts';
...
it('opens a fresh daily counter while preserving monthly commitments across UTC rollover', async () => {
    ...
    try {
        ...
        await store.run(`INSERT INTO workday_capacity_envelopes (id, team_id, project_id, allocation_set_id, status, started_at, envelope_json, metadata_json, created_at, updated_at) VALUES ('workday-a', 'team-a', 'project-a', 'allocation-a', 'active', ?, '{"availableSeconds":10}', '{}', ?, ?)`, [beforeMidnight, beforeMidnight, beforeMidnight]);
        ...
        await settleCapacityReservationExactlyOnce(store, {
            ...
            reservationId: 'reservation-before-rollover', assignmentId: 'assignment-before-rollover', activeSeconds: 4, elapsedSeconds: 4, source: 'test',
        });
        ...
        after.grantCommitted.dailyAgentSeconds = 0;
        await commitCapacityAdmission(store, {
        ...
        expect(await aggregateCapacityTimeReservations(store, { teamId: 'team-a', now: afterMidnight })).toMatchObject({
            activeReservedSeconds: 6,
            dailyActiveSeconds: 0,
            dailyCommittedSeconds: 6,
            monthlyActiveSeconds: 4,
            monthlyCommittedSeconds: 10,
...
it('leases only admitted membership work with CAS and releases it when membership authority is suspended', async () => {
    ...
    try {
        ...
        await settleCapacityReservationExactlyOnce(store, { settlementKey: 'lease-settle-a', teamId: 'team-a', membershipId: 'membership-a', reservationId: 'lease-reservation-a', assignmentId: 'lease-assignment-a', activeSeconds: 4, elapsedSeconds: 4, source: 'test' });
...
it('changes real admission when a replacement stored allocation policy is activated', async () => {
    ...
    try {
        ...
        await store.run(`INSERT INTO workday_capacity_envelopes (id, team_id, project_id, allocation_set_id, status, started_at, envelope_json, metadata_json, created_at, updated_at) VALUES ('workday-policy', 'team-a', 'project-a', 'allocation-a', 'active', ?, '{"availableSeconds":10}', '{"grantId":"grant-a"}', ?, ?)`, [now, now, now]);
        await store.run(`INSERT INTO capacity_provider_availability_sessions (id, membership_id, team_id, capacity_provider_id, environment, status, sequence, opened_at, refreshed_at, expires_at, available_from, available_until, execution_providers_json, capabilities_json, native_limits_json, runner_pressure_json, constraints_json, metadata_json, created_at, updated_at) VALUES ('session-policy', 'membership-a', 'team-a', 'provider-a', 'local', 'open', 1, ?, ?, ?, ?, ?, '[{"id":"codex"}]', '["engineering"]', '{"availableAgentSeconds":10,"maxConcurrentRunners":1}', '{"activeRunners":0,"maxConcurrentRunners":1}', '{"availableAgentSeconds":10,"activeRunners":0,"maxConcurrentRunners":1}', '{}', ?, ?)`, [now, now, new Date(Date.now() + 60000).toISOString(), now, new Date(Date.now() + 60000).toISOString(), now, now]);
        const request = { teamId: 'team-a', providerId: 'provider-a', membershipId: 'membership-a', projectId: 'project-a', environment: 'local', projectAgentClassId: 'class-a', mode: 'planning' as const, workDayId: 'workday-policy', requestedSeconds: 6, executionProviderId: 'codex', laneId: 'lane-a', providerSessionId: 'session-policy', requiredCapabilities: ['engineering'] };
        ...
        expect(evaluateCapacityAdmission(allowedState)).toMatchObject({ allowed: true, allocationSetId: replacement.id, maxReservableSeconds: 10 });
...
it('commits stored sibling borrowing with donor, recipient, overflow, and rule counters', async () => {
    ...
    try {
        ...
        await store.run(`UPDATE capacity_grants SET daily_agent_seconds_limit = 100, monthly_agent_seconds_limit = 100, max_concurrent_assignments = 2 WHERE id = 'grant-a'`);
        await store.run(`INSERT INTO workday_capacity_envelopes (id, team_id, project_id, allocation_set_id, status, started_at, envelope_json, metadata_json, created_at, updated_at) VALUES ('workday-borrow', 'team-a', 'project-a', 'allocation-a', 'active', ?, '{"availableSeconds":100}', '{"grantId":"grant-a"}', ?, ?)`, [now, now, now]);
        await store.run(`INSERT INTO capacity_provider_availability_sessions (id, membership_id, team_id, capacity_provider_id, environment, status, sequence, opened_at, refreshed_at, expires_at, available_from, available_until, execution_providers_json, capabilities_json, native_limits_json, runner_pressure_json, constraints_json, metadata_json, created_at, updated_at) VALUES ('session-borrow', 'membership-a', 'team-a', 'provider-a', 'local', 'open', 1, ?, ?, ?, ?, ?, '[{"id":"codex"}]', '["engineering"]', '{"availableAgentSeconds":100,"maxConcurrentRunners":2}', '{"activeRunners":0,"maxConcurrentRunners":2}', '{"availableAgentSeconds":100,"activeRunners":0,"maxConcurrentRunners":2}', '{}', ?, ?)`, [now, now, new Date(Date.now() + 60000).toISOString(), now, new Date(Date.now() + 60000).toISOString(), now, now]);
        ...
        const state = await loadCapacityAdmissionState(store, { teamId: 'team-a', providerId: 'provider-a', membershipId: 'membership-a', projectId: 'project-a', environment: 'local', projectAgentClassId: 'class-a', mode: 'planning', workDayId: 'workday-borrow', requestedSeconds: 10, executionProviderId: 'codex', laneId: 'lane-a', providerSessionId: 'session-borrow', requiredCapabilities: ['engineering'] });
        const decision = evaluateCapacityAdmission(state);
        expect(decision).toMatchObject({ allowed: true, maxReservableSeconds: 15 });
