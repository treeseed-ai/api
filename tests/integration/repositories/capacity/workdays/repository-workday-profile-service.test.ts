...
const bundle=(profiles=[profile])=>({schemaVersion:'treeseed.workday-allocation-profile-bundle/v1',profiles});
...
class FakeDatabase {
	...
	async first<T extends Row=Row>(query:string,params:unknown[]=[]):Promise<T|null> {
		...
		if(query.includes('FROM capacity_allocation_sets')&&query.includes('status')) return this.allocations.find((entry)=>entry.id===params[1]) as T??null;
	...
	async all<T extends Row=Row>(query:string):Promise<T[]> {
		...
		if(query.includes('FROM capacity_operation_receipts')) return [...this.receipts].reverse() as T[];
		return [];
	}
	async batch(operations:CapacityDatabaseOperation[]) {
		for(const operation of operations) {
			...
			if(operation.query.includes("UPDATE capacity_allocation_sets SET status='superseded'")) {
				const allocation=this.allocations.find((entry)=>entry.id===operation.params?.[3]);
				if(allocation) Object.assign(allocation,{status:'superseded',superseded_by_id:operation.params?.[0]});
			}
			if(operation.query.includes("UPDATE capacity_allocation_sets SET status='archived'")) {
				const allocation=this.allocations.find((entry)=>entry.id===operation.params?.[2]);
				if(allocation) Object.assign(allocation,{status:'archived'});
			}
...
function observation(overrides:Record<string,unknown>={}) { return {repository:'treeseed-ai/sdk',ref:'refs/heads/staging',commit,path:REPOSITORY_WORKDAY_PROFILE_PATH,content:JSON.stringify(bundle()),observedAt:'2026-08-21T09:00:00.000Z',...overrides}; }
...
describe('repository workday profile indexing',()=>{
	...
	it('creates a superseding allocation generation when normalized profile content changes',async()=>{
		...
		const [second]=await service.reconcile(observation({commit:'d'.repeat(40),content:JSON.stringify(bundle([changed])),observedAt:'2026-08-21T09:10:00.000Z'}));
		...
		expect(database.allocations.map((entry)=>entry.status)).toEqual(['superseded','active']);
	});

	it('indexes multiple current profiles without superseding sibling profiles',async()=>{
		const database=new FakeDatabase();
		const profiles=['feature','stability','documentation','testing'].map((id)=>({...profile,id:`sdk-${id}`}));
		const receipts=await new RepositoryWorkdayProfileService(database as unknown as CapacityGovernanceDatabase).reconcile(observation({content:JSON.stringify(bundle(profiles))}));
		expect(receipts).toHaveLength(4);
		expect(receipts.map((entry)=>String((entry as any).generation.profileId))).toEqual(['sdk-documentation','sdk-feature','sdk-stability','sdk-testing']);
		expect(database.allocations).toHaveLength(4);
		expect(database.allocations.every((entry)=>entry.status==='active')).toBe(true);
	});

	it('archives a profile removed from the next accepted repository bundle',async()=>{
		const database=new FakeDatabase();
		const service=new RepositoryWorkdayProfileService(database as unknown as CapacityGovernanceDatabase);
		await service.reconcile(observation({content:JSON.stringify(bundle([profile,{...profile,id:'sdk-stability'}]))}));
		await service.reconcile(observation({commit:'c'.repeat(40),content:JSON.stringify(bundle([profile])),observedAt:'2026-08-21T09:15:00.000Z'}));
		expect(database.allocations).toHaveLength(2);
		expect(database.allocations.find((entry)=>String(JSON.parse(String(entry.metadata_json)).workdayProfile.id)==='sdk-stability')).toMatchObject({status:'archived'});
		expect(database.allocations.find((entry)=>String(JSON.parse(String(entry.metadata_json)).workdayProfile.id)==='sdk-balanced')).toMatchObject({status:'active'});
	});
