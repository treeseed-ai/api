...
import { loadInfrastructureSeedState } from '../../../../src/market/seeds/infrastructure-seeds.js';
...
const PROJECT_SLUGS = ['admin', 'agent', 'api', 'cli', 'core', 'market', 'sdk', 'treedx', 'ui'];
const PACKAGE_SLUGS = ['admin', 'agent', 'api', 'cli', 'core', 'sdk', 'treedx', 'ui'];
...
describe('local seed apply', () => {
	...
	it('targets the Treeseed PostgreSQL database and attaches the local owner', async () => {
		...
		try {
			...
			expect(projects.map((project: any) => project.slug).sort()).toEqual(PROJECT_SLUGS);
		} finally {
			db.close();
		}
	}, 15000);
