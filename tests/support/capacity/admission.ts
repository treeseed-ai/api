...
export function createCapacityAdmissionTestHarness() {
	...
	const store = createCapacityControlPlane(new ControlPlaneStore({ repoRoot: packageRoot }, database));
	return { database, store };
}

export function capacityAdmissionInput(requestedSeconds: number, committed = 0, now = new Date().toISOString()): CapacityAdmissionInput {
	const nowMs = Date.parse(now);
	return {
		now,
		request: { teamId: 'team-a', providerId: 'provider-a', membershipId: 'membership-a', projectId: 'project-a', environment: 'local', agentClassId: 'class-a', mode: 'planning', executionProviderId: 'codex', laneId: 'lane-a', requiredCapabilities: ['engineering'], requestedSeconds },
		...
		grant: { schemaVersion: 2, id: 'grant-a', membershipId: 'membership-a', teamId: 'team-a', providerId: 'provider-a', projectId: 'project-a', environment: 'local', status: 'active', executionProviderIds: ['codex'], laneIds: ['lane-a'], capabilities: ['engineering'], allowedModes: ['planning'], dailyAgentSecondsLimit: 10, monthlyAgentSecondsLimit: 20, maxConcurrentAssignments: 1, unmetered: false },
		workday: { id: 'workday-a', status: 'active', totalSeconds: 10, committedSeconds: committed },
		...
		committedSecondsBySlice: { 'project:project-a': committed },
		providerCapacity: { availableAgentSeconds: 10 - committed, availableConcurrentAssignments: 1 },
		providerLocalLimits: { availableAgentSeconds: 10 - committed, availableConcurrentAssignments: 1 },
		grantCommitted: { dailyAgentSeconds: committed, monthlyAgentSeconds: committed, activeAssignments: 0 },
	};
}
