...
describe('discussion catalog operations', () => {
	it('binds list, create, and lifecycle changes without HTTP knowledge', async () => {
		const discussions = {
			...
			updateStatus: vi.fn(async () => ({ discussionId: 'discussion-1', status: 'closed' })),
		};
		...
		await operations[2].handler({ path: { discussionId: 'discussion-1' }, query: {}, body: { projectId: 'project-1', status: 'closed' } }, context);
		expect(discussions.updateStatus).toHaveBeenCalledWith(context.principal, 'discussion-1',
			{ projectId: 'project-1', status: 'closed' }, 'key-1');
	});
});
