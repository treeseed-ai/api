...
describe('governance catalog read operations', () => {
	it('binds proposal and decision reads without transport knowledge', async () => {
		...
		await operations[9].handler({ path: { projectId: 'project-1', proposalId: 'proposal-1' }, query: {}, body: undefined }, context);
		expect(governance.proposal).toHaveBeenCalledWith(context.principal, 'project-1', 'proposal-1');
		await operations[1].handler({ path: { projectId: 'project-1', proposalId: 'proposal-1' }, query: {}, body: { title: 'Updated' } },
			{ ...context, ifMatch: '3' });
