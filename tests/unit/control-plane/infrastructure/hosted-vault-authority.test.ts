...
describe('hosted service-vault authority', () => {
	...
	it('derives exact state-session and encryption profiles only from the team storage connection', async () => {
		...
		expect(store.first.mock.calls.map((call) => call[1])).toEqual([
			['object-storage', 'team-1', 'cloudflare-state', 'cloudflare-state', 'cloudflare', 's3-state-session'],
			['state-encryption', 'team-1', 'cloudflare-state', 'cloudflare-state', 'cloudflare', 'opentofu-state-encryption'],
		]);
