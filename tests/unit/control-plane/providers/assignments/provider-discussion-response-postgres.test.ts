...
describe.skipIf(!process.env.TREESEED_TEST_POSTGRES_URL)('discussion publication SQL authority', () => {
	it('retains the exact live lease and blocks stale or cross-team response attribution in PostgreSQL', async () => {
		...
		try {
			...
			await client.query(`CREATE TEMP TABLE capacity_provider_assignments (id text,team_id text,status text,lease_state text,lease_token text);
				CREATE TEMP TABLE agent_invocation_requests (id text,team_id text,assignment_id text,status text,final_message_ref text,response_json jsonb,updated_at text);
				INSERT INTO capacity_provider_assignments VALUES ('assignment','team','leased','leased','lease');
				INSERT INTO agent_invocation_requests VALUES ('invocation','team','assignment','running',NULL,NULL,NULL);`);
			const store = {
			...
			await client.query("UPDATE capacity_provider_assignments SET status='expired',lease_token=NULL");
