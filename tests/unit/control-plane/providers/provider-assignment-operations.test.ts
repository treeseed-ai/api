...
function dependencies() {
	return {
		providerAssignments: { next: vi.fn(), show: vi.fn(), explain: vi.fn(), renew: vi.fn(), startExecution: vi.fn(), startCloseout: vi.fn(),
			preflight: vi.fn(), respondToDiscussion: vi.fn(), returnAssignment: vi.fn(), complete: vi.fn(), fail: vi.fn(), reportUsage: vi.fn(), settle: vi.fn(), createModeRun: vi.fn(), createEvent: vi.fn() },
...
describe('provider assignment operation catalog', () => {
	it('binds all eighteen runtime operations to exact SDK objects', () => {
