...
describe('protected diagnostic envelopes', () => {
	it('authenticates full correlation and rejects row substitution', () => {
		const service = createDiagnosticEnvelopeService({ environment: 'test', TREESEED_DIAGNOSTICS_ENCRYPTION_KEY: 'diagnostic-test-key-material-generation-one', TREESEED_DIAGNOSTICS_KEY_VERSION: 1 });
	...
	it('rewraps only the DEK under an active generation while preserving payload ciphertext', () => {
		const old = createDiagnosticEnvelopeService({ environment: 'test', TREESEED_DIAGNOSTICS_ENCRYPTION_KEY: 'diagnostic-test-key-material-generation-one', TREESEED_DIAGNOSTICS_KEY_VERSION: 1 });
		...
		writeFileSync(prior, 'diagnostic-test-key-material-generation-one');
		const next = createDiagnosticEnvelopeService({ environment: 'test', TREESEED_DIAGNOSTICS_ENCRYPTION_KEY: 'diagnostic-test-key-material-generation-two', TREESEED_DIAGNOSTICS_KEY_VERSION: 2, TREESEED_DIAGNOSTICS_HISTORICAL_KEY_FILES: `1:${prior}` });
