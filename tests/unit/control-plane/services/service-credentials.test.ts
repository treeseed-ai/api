...
function fixture() {
  const connection = {id:'connection-1',teamId:'team-1',providerId:'cloudflare',status:'active',
    nonSecretConfig:{},capabilities:[{capabilityType:'object-storage',credentialProfileId:'cloudflare-storage',status:'configured'}]};
  ...
  const session = vi.fn(async (scope:any,run:any)=>{expect(canonicalSecretPath(scope)).toBe('teams/team-1/projects/team/environments/shared/purposes/cloudflare-storage/secrets/connection-1');return run(custody);});
...
describe('managed service credentials',()=>{
  ...
  it('returns only metadata, including optional fields; rotates with CAS and supports tombstone then recreation',async()=>{
    ...
    expect(receipt).toMatchObject({custody:'openbao',version:1,configured:true,fields:['apiToken']});
    ...
    expect(await service.putCredentials(principal,...args,{expectedVersion:1,values:{apiToken:'secret'}})).toMatchObject({version:2});
  });
  ...
  it('rejects wrong team, missing capability, omitted fields and unrelated fields before any secret write',async()=>{
    const f=fixture();
    await expect(f.service.putCredentials(principal,...args,{expectedVersion:0,values:{}})).rejects.toMatchObject({status:400});
    await expect(f.service.putCredentials(principal,...args,{expectedVersion:0,values:{apiToken:'secret',unexpected:'no'}})).rejects.toMatchObject({status:400});
  ...
  it('does not misreport provider validation failure as a custody outage',async()=>{
    const f=fixture();
    await f.service.putCredentials(principal,...args,{expectedVersion:0,values:{apiToken:'fixture-secret'}});
