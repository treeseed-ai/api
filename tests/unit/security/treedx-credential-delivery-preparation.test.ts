	...
	it('reconciles first-party authority to the central token and returns only an opaque delivery id', async () => {
		...
		const store = {
			first: vi.fn().mockImplementation(async (sql: string) => {
				if (sql.includes('FROM projects p JOIN teams')) return { id: 'project-1', team_id: 'team-1', slug: 'api-api' };
				if (sql.includes('FROM team_service_connections')) return {
					...
					authority_reference: 'TREESEED_GITHUB_TOKEN_KNOWLEDGE_COOP_API',
				};
				if (sql.includes('FROM provider_credential_authorities a')) return {
					...
					status: 'ready', non_secret_config_json: '{}', owner: 'treeseed-ai', name: 'api-api-content',
				};
		...
		const fetchImpl = vi.fn().mockImplementation(async (url: string) => {
			const payload = url.includes('/git/ref/heads/')
				? { object: { sha } }
				: { id: 42, name: 'api-api-content', owner: { login: 'treeseed-ai' }, archived: false, disabled: false };
		...
		const result = await prepareTreeDxCredentialDelivery({
			...
			body: {
				teamSlug: 'treeseed', projectSlug: 'api-api', owner: 'treeseed-ai', name: 'api-api-content',
