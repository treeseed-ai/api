...
function store(diagnostics: ProjectCapacityDiagnostics, allocationSet: CapacityAllocationSetV2 | null) {
	...
	const all = vi.fn(async (query: string, _params?: unknown[]) => {
	...
	return {
		...
		all: async <T extends Record<string, unknown>>(query: string, params?: unknown[]) => await all(query, params) as unknown as T[],
		first: async <T extends Record<string, unknown>>(query: string, params?: unknown[]) => await first(query, params) as unknown as T | null,
	};
}
