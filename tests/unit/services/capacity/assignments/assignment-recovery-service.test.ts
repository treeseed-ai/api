...
function harness() {
	...
	const host = new ControlPlaneStore({ repoRoot: packageRoot }, database);
...
async function seed(store: ReturnType<typeof harness>['store'], now: string) {
	...
	await store.run(`INSERT INTO workday_capacity_envelopes (id, team_id, project_id, allocation_set_id, status, started_at, envelope_json, metadata_json, created_at, updated_at) VALUES ('workday-a', 'team-a', 'project-a', 'allocation-a', 'active', ?, '{"availableSeconds":100}', '{"grantId":"grant-a"}', ?, ?)`, [now, now, now]);
}
...
describe('expired assignment recovery', () => {
	...
	it('converges a provider failure interrupted after settlement but before lifecycle transition', async () => {
		...
		try {
			...
			await expect(interrupted.failProviderAssignment(
				...
				{ leaseToken: 'lease-interrupted-provider-failure', activeSeconds: 1, elapsedSeconds: 1, code: 'execution_failed', retryable: false },
			...
			expect(await store.first(`SELECT state, active_seconds FROM capacity_reservations WHERE id = 'reservation-interrupted-provider-failure'`)).toEqual({ state: 'consumed', active_seconds: 1 });
	...
	it('classifies safe retry, exhausted retry, recovered completion, and operator action exactly once', async () => {
		...
		try {
			...
			await store.run(`INSERT INTO capacity_workday_runs (id, team_id, capacity_provider_id, scenario_id, status, environment, parameters_json, started_at, created_at, updated_at) VALUES ('run-a', 'team-a', 'provider-a', 'planning', 'running', 'local', '{}', ?, ?, ?)`, [now, now, now]);
			await store.run(`UPDATE workday_capacity_envelopes SET workday_run_id = 'run-a' WHERE id = 'workday-a'`);
			...
			await reportCapacityUsage(store, { teamId: 'team-a', membershipId: 'membership-a', reservationId: operatorReservation, assignmentId: 'operator', idempotencyKey: 'operator-usage', usageDimension: 'tool-events', accountingMode: 'informational', activeSeconds: 0, elapsedSeconds: 0, source: 'test' });
			await store.createAgentModeRun({ id: 'operator-run', teamId: 'team-a', providerAssignmentId: 'operator', status: 'running' });
			await settleCapacityReservationExactlyOnce(store, { settlementKey: 'completed-settlement', teamId: 'team-a', membershipId: 'membership-a', reservationId: completedReservation, assignmentId: 'completed', activeSeconds: 1, elapsedSeconds: 1, source: 'provider_usage_report' });
			await store.createAgentModeRun({ id: 'completed-run', teamId: 'team-a', providerAssignmentId: 'completed', status: 'succeeded' });
			await settleCapacityReservationExactlyOnce(store, { settlementKey: 'intermediate-settlement', teamId: 'team-a', membershipId: 'membership-a', reservationId: intermediateReservation, assignmentId: 'intermediate', activeSeconds: 0, elapsedSeconds: 0, source: 'expired_lease_recovery' });
			await settleCapacityReservationExactlyOnce(store, { settlementKey: 'provider-failure-settlement', teamId: 'team-a', membershipId: 'membership-a', reservationId: providerFailureReservation, assignmentId: 'provider-failure', activeSeconds: 1, elapsedSeconds: 1, source: 'provider_assignment_fail' });
			...
			expect(await store.first(`SELECT status, lease_state, attempt_count, lifecycle_code FROM capacity_provider_assignments WHERE id = 'safe'`)).toEqual({ status: 'failed', lease_state: 'released', attempt_count: 1, lifecycle_code: 'expired_lease_requeued' });
			expect(await store.first(`SELECT status, assignment_id FROM capacity_workday_demands WHERE id = 'demand-safe'`)).toEqual({ status: 'pending', assignment_id: null });
