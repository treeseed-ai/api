...
import type { DurableProviderAssignment } from '../../../src/api/capacity/repositories/assignment.ts';
import { resolvePlanningDemandSource } from '../../../src/api/capacity/services/planning-demand-source.ts';
import { projectCompletedResearchWorkflow, type ResearchWorkflowProjectionStore } from '../../../src/api/capacity/services/research-workflow-projection-service.ts';
import type { CapacityWorkdayAgent } from '../../../src/api/capacity/services/workday-agent-policy.ts';
...
const researcher = {
	slug: 'researcher', handler: 'writer', projectAgentClassId: 'class-research', projectAgentClassSlug: 'research',
	purpose: 'Research', promptTask: 'planning', outputContract: {}, planningIntent: {}, planningPriority: 1,
	planningAllocationPercent: null, activityType: 'planning',
} satisfies CapacityWorkdayAgent;
function projectionStore(
	store: ReturnType<typeof harness>['store'],
	payload: () => Record<string, unknown>,
): ResearchWorkflowProjectionStore {
	return {
		first: async <T extends Record<string, unknown>>() => ({ id: 'demand-a', payload_json: JSON.stringify(payload()) }) as unknown as T,
...
describe('durable research workflow service', () => {
	...
	it('advances a ready stage from canonical assignment artifact evidence and replays idempotently', async () => {
		...
		try {
			await seed(store);
			const workflow = await store.createResearchWorkflow('project-a', { id: 'research-a', objectiveRef: 'objective:a', questionRef: 'question:a', idempotencyKey: 'research:a' });
			const assignment = { id: 'assignment-a', teamId: 'team-a', projectId: 'project-a', mode: 'planning' } as DurableProviderAssignment;
			const projection = projectionStore(store, () => ({ researchWorkflowId: workflow!.id, researchWorkflowStateVersion: 1, researchStage: 'question-decomposition' }));
			...
			const advanced = await projectCompletedResearchWorkflow(projection, assignment, input);
			...
			await expect(projectCompletedResearchWorkflow(projection, assignment, input)).resolves.toMatchObject({ stateVersion: 2 });
		} finally { await database.close(); }
	});
	...
	it('preserves prior citations when a later artifact manifest declares none', async () => {
		...
		try {
			...
			const assignment = { id: 'assignment-evidence', teamId: 'team-a', projectId: 'project-a', mode: 'planning' } as DurableProviderAssignment;
			const projection = projectionStore(store, () => ({ researchWorkflowId: workflow!.id, researchWorkflowStateVersion: workflow!.stateVersion, researchStage: 'linked-evidence-notes' }));
			const input = { artifactManifest: {
			...
			await expect(projectCompletedResearchWorkflow(projection, assignment, input)).resolves.toMatchObject({ citations: expect.arrayContaining([expect.objectContaining({ publisher: 'one.test' }), expect.objectContaining({ publisher: 'two.test' })]) });
		} finally { await database.close(); }
	});
	...
	it('projects a post-revision rejection into another durable revision cycle', async () => {
		...
		try {
			...
			const assignment = { id: 'assignment-review-again', teamId: 'team-a', projectId: 'project-a', mode: 'planning' } as DurableProviderAssignment;
			const projection = projectionStore(store, () => ({ researchWorkflowId: workflow!.id, researchWorkflowStateVersion: workflow!.stateVersion, researchStage: 'citation-review-approval' }));
			const input = { artifactManifest: {
			...
			await expect(projectCompletedResearchWorkflow(projection, assignment, input)).resolves.toMatchObject({
